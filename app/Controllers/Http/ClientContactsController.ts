...
export default class ClientContactsController {
  ...
  public async client ({view, params}: HttpContextContract) {
    ...
    const client = await Client.query().where('id', params.client_id)

    return view.render('admin/client_contacts', {clientContacts, client_id: params.client_id, client,client_T : client_type[client[0].client_type_id] })
  }
